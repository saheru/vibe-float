...
- StreamDock Vibe Control の全アイコンを ImageGen で再設計しました。Codex は寒色系ブルー、Claude は暖色系オレンジとし、タスク、モデル、権限、Sol Effort、Usage に個別の機能アイコンを用意しました。
- Spotify Control v0.3.2 にも統一されたグリーンの音楽操作アイコンを導入し、現在の曲、前／次の曲、音量を明確に区別しました。
- すべてのアイコンを透過処理し、小さいデバイス表示向けに中央配置と視認性を最適化しました。

## [0.5.7] - 2026-08-04

### 修正

- Codex CLI 0.146.0 で必須になった App Server 初期化 capability を追加し、macOS の JSON-RPC を WebSocket テキストフレームへ変更しました。接続失敗と、macOS の最近のタスクが Claude だけになり Codex 操作がオフラインになる問題を修正しました。
